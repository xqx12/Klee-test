		...
		case Instruction::Call: 
			{
				...
				klee_message("[xqx]: call fp func = %s ", fp->getName().data());
				Instruction * lastInst;
				const InstructionInfo &iii = getLastNonKleeInternalInstruction(state, &lastInst);
				if (iii.file != "") {
					klee_message("[xqx]: %s:%d ", iii.file.c_str(), iii.line);
				} else {
					klee_message("[xqx]: (location information missing) ");
				}
		...
		case Instruction::Alloca: 
			...
			/* 	klee_message("[xqx]===exec inst=================");
				const InstructionInfo &ii = *state.pc->info;
					...
					//info << "lines: " << &ii.file.c_str() << ":" << ii.line << "\n";
				...
				Instruction * lastInst;
				const InstructionInfo &iii = getLastNonKleeInternalInstruction(state, &lastInst);
				//FIX: why not have file info, but is ok in terminateStateOnError.
				if (iii.file != "") {
					klee_message("[xqx]: %s:%d ", iii.file.c_str(), iii.line);
				} else {
					klee_message("[xqx]: (location information missing) ");
...
void Executor::executeAlloc(ExecutionState &state,
		...
		/* 			//addbyxqx201402
				...
				//FIXME: why not have file info, but is ok in terminateStateOnError.
				//printFileLine(state, iii);
					std::ostringstream stackinfo;
					stackinfo << "Stack: \n";
					state.dumpStack(stackinfo);
					const std::string tmp = stackinfo.str();
					const char* cstr = tmp.c_str();
					klee_message("[xqx]: %s", cstr);
	...
	if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
		...
		if (fixedSize.second) { 
			...
			(void) success;
			/* 
				klee_message("[xqx]---the fixsize.second-----------------");
				tmp->dump();
				klee_message("[xqx]===the fixsize.second=================");
			*/
			bool res;
			...
			if (res) {
				...
				/* 
				klee_message("[xqx]===the fixsize.second not exist, check the max size===");
				UltExpr::create(ConstantExpr::alloc(1<<31, W), size)->dump();
				ref<Expr> isZeroSize = EqExpr::create(size, ConstantExpr::create(100,W));
				ref<ConstantExpr> tmpZeroValue;
				
				isZeroSize->dump();
				bool bZero ;
				success = solver->mayBeTrue(*fixedSize.second, isZeroSize, bZero);
				assert(success && "FIXME: Unhandled solver failure");      
				(void) success;
				if(!bZero)
					klee_message( "[xqx] NOT be Zero");      
				if (isa<ConstantExpr>(isZeroSize))
					klee_message( "[xqx] isZeroSize is ConstantExpr");      
				addConstraint(*fixedSize.second, isZeroSize);
				std::ostringstream xinfo;
				ExprPPrinter::printOne(xinfo, "  size expr", size);
				xinfo << "Stack: \n";
				(*fixedSize.second).dumpStack(xinfo);
		...
		{
			//klee_message("[xqx]---the fixsize.first-----------------");
			//example->dump();
			//klee_message("[xqx]===the fixsize.first=================");
...
/* 
 * ===  FUNCTION  ======================================================================
 *         Name:  doSizeControlledMalloc
 *  Description:  
 *  Copyright: addbyxqx 2014年02月17日 15时32分30秒
 * =====================================================================================
 */
		...
		ref<Expr> size	)
{
	...
	CallSite cs(i);
	Value *fp = cs.getCalledValue();
	Function *fc = getTargetFunction(fp, state);
	if(!fc){
